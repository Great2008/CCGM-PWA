...
// ─── Verse-reference parsing (for "open in Bible" links) ──────────────────────
// Book names in refs mostly match KJV_BOOKS' `name` field exactly, except
// "Psalm" (singular, as commonly written in a reference) vs. "Psalms" (the
// book's actual name) — aliased below.
const BOOK_NAME_TO_ID = (() => {
  const map = {}
  KJV_BOOKS.forEach(b => { map[b.name.toLowerCase()] = b.id })
  map['psalm'] = 'PSA'
  return map
})()

// "Jeremiah 17:14" -> {bookId:'JER', ch:17, v:14}
// "James 5:14–15"  -> {bookId:'JAS', ch:5, v:14}  (opens at the first verse of a range)
function parseRef(ref) {
  const m = ref.match(/^(.+?)\s+(\d+):(\d+)/)
  if (!m) return null
  const bookId = BOOK_NAME_TO_ID[m[1].trim().toLowerCase()]
  if (!bookId) return null
  return { bookId, ch: parseInt(m[2], 10), v: parseInt(m[3], 10) }
}
...
function VerseCard({ verse, isFav, onToggleFav, searchQuery = '', onOpenVerse }) {
  const [copied, setCopied] = useState(false)
  const parsed = useMemo(() => parseRef(verse.ref), [verse.ref])

  const handleOpen = () => {
    if (parsed && onOpenVerse) onOpenVerse(parsed.bookId, parsed.ch, parsed.v)
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
        <span
          onClick={parsed ? handleOpen : undefined}
          title={parsed ? `Read ${verse.ref} in the Bible` : undefined}
          style={{
...
export default function PrayerTab({ onOpenVerse }) {
