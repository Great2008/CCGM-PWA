...
export default function AdminBranches() {
  ...
  const [sugTab, setSugTab] = useState('branches')
  ...
  const dismissSuggestion = async (id) => {
    ...
    showToast('Suggestion dismissed.'); loadSuggestions()
  }

  const sugBtnLabel = suggestions.length ? ('📬 Suggestions (' + suggestions.length + ')') : '📬 Suggestions'
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        ...
        {sugTab === 'branches' && (
          <button onClick={openNew} style={{ padding: '10px 22px', borderRadius: 40, background: 'linear-gradient(135deg,var(--brand-base),var(--brand-mid))', color: 'white', fontWeight: 700, fontSize: '0.86rem', fontFamily: 'var(--font-body)', border: 'none', cursor: 'pointer', boxShadow: 'var(--shadow-sm)' }}>
            + Add Branch
          </button>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        <button onClick={() => setSugTab('branches')} style={{ padding: '8px 20px', borderRadius: 30, border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: '0.84rem', background: sugTab === 'branches' ? 'var(--brand-mid)' : '#f1f5f9', color: sugTab === 'branches' ? 'white' : 'var(--text-mid)' }}>
          ⛪ All Branches
        </button>
        <button onClick={() => setSugTab('suggestions')} style={{ padding: '8px 20px', borderRadius: 30, border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: '0.84rem', background: sugTab === 'suggestions' ? 'var(--brand-mid)' : '#f1f5f9', color: sugTab === 'suggestions' ? 'white' : 'var(--text-mid)' }}>
          {sugBtnLabel}
        </button>
      </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
                    <div>
                      ...
                      <div style={{ color: 'var(--text-light)', fontSize: '0.8rem' }}>
                        Submitted by: <strong>{sug.profiles ? (sug.profiles.display_name || sug.profiles.full_name || 'Unknown') : 'Unknown'}</strong>
                        {sug.profiles && sug.profiles.email ? ' (' + sug.profiles.email + ')' : ''}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                      <button onClick={() => approveSuggestion(sug)} style={{ padding: '8px 18px', borderRadius: 8, border: 'none', background: 'var(--brand-pale)', color: 'var(--brand-mid)', fontWeight: 700, fontSize: '0.82rem', fontFamily: 'var(--font-body)', cursor: 'pointer' }}>
                        ✅ Add Branch
                      </button>
                      <button onClick={() => dismissSuggestion(sug.id)} style={{ padding: '8px 14px', borderRadius: 8, border: '1px solid #fecaca', background: 'transparent', color: '#dc2626', fontWeight: 600, fontSize: '0.82rem', fontFamily: 'var(--font-body)', cursor: 'pointer' }}>
                        Dismiss
                      </button>
                    </div>
                  </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16, marginBottom: 18 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', paddingTop: 6 }}>
                    <input type="checkbox" checked={form.active} onChange={e => setForm(f => ({ ...f, active: e.target.checked }))} style={{ width: 18, height: 18, accentColor: 'var(--brand-base)', cursor: 'pointer' }} />
                    <span style={{ fontSize: '0.9rem', color: 'var(--text-dark)', fontWeight: 600 }}>Active (shows in dropdown)</span>
                  </label>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={handleSave} disabled={saving} style={{ padding: '10px 26px', borderRadius: 40, background: saving ? '#9ca3af' : 'var(--brand-mid)', color: 'white', fontWeight: 700, fontSize: '0.86rem', fontFamily: 'var(--font-body)', border: 'none', cursor: saving ? 'not-allowed' : 'pointer' }}>
                  {saving ? 'Saving…' : editing ? 'Update Branch' : 'Add Branch'}
                </button>
                <button onClick={cancel} style={{ padding: '10px 22px', borderRadius: 40, border: '1.5px solid #e2e8f0', background: 'transparent', color: 'var(--text-mid)', fontWeight: 600, fontSize: '0.86rem', fontFamily: 'var(--font-body)', cursor: 'pointer' }}>
                  Cancel
                </button>
              </div>
            </div>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {branches.map((b, i) => (
                      <tr key={b.id}
                        style={{ borderBottom: i < branches.length - 1 ? '1px solid #f1f5f9' : 'none', transition: 'background 0.15s' }}
                      ...
                      >
                        ...
                        <td style={{ padding: '14px 18px' }}>
                          <button onClick={() => toggleActive(b)} style={{ fontSize: '0.72rem', fontWeight: 700, padding: '3px 12px', borderRadius: 20, border: 'none', cursor: 'pointer', background: b.active !== false ? 'var(--brand-pale)' : '#f3f4f6', color: b.active !== false ? 'var(--brand-mid)' : '#9ca3af', fontFamily: 'var(--font-body)' }}>
                            {b.active !== false ? '✅ Active' : '⏸ Inactive'}
                          </button>
                        </td>
                        <td style={{ padding: '14px 18px' }}>
                          <div style={{ display: 'flex', gap: 8 }}>
                            <button onClick={() => openEdit(b)} style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid #e2e8f0', background: 'transparent', color: 'var(--brand-mid)', fontSize: '0.8rem', fontWeight: 600, fontFamily: 'var(--font-body)', cursor: 'pointer' }}>Edit</button>
                            <button onClick={() => handleDelete(b.id)} style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid #fecaca', background: 'transparent', color: '#dc2626', fontSize: '0.8rem', fontWeight: 600, fontFamily: 'var(--font-body)', cursor: 'pointer' }}>Delete</button>
                          </div>
                        </td>
                      </tr>
                    ))}
