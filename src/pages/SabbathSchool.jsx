...
export default function SabbathSchool() {
  ...
  if (loading) return (
    <div style={{minHeight:'60vh',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:12}}>
      <div style={{fontSize:'2rem',animation:'ptr-spin 1.5s linear infinite'}}>📖</div>
  ...
  const tabs = [
    ...
    { id: 'divine',    label: 'Service',   icon: 'church' },
  ]
    ...
    <div>
        ...
        <div style={{background:'linear-gradient(135deg,var(--brand-deep),var(--brand-mid))',padding:'clamp(90px,14vw,130px) 5% 56px',textAlign:'center'}}>
          ...
          <p style={{color:'rgba(255,255,255,0.75)',maxWidth:520,margin:'0 auto',lineHeight:1.8,fontSize:'0.95rem'}}>
            Weekly lessons to deepen your understanding of God Word. Study along with our community every Sabbath.
          </p>
              ...
              <div style={{position:'relative'}}>
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:16,flexWrap:'wrap'}}>
                  <span style={{background:'var(--gold)',color:'var(--brand-deep)',padding:'4px 14px',borderRadius:20,fontSize:'0.72rem',fontWeight:900,letterSpacing:'0.1em',textTransform:'uppercase'}}>
                    {selected.lesson_date === lessons[0]?.lesson_date ? "This Week Lesson" : "Latest Lesson"}
                  </span>
              ...
              <div style={{background:'white',borderRadius:16,boxShadow:'var(--shadow-sm)',border:'1.5px solid #e2e8f0',overflow:'hidden'}}>
                ...
                <div style={{maxHeight:520,overflowY:'auto'}}>
                  ...
                  {filtered.map(l => {
                    ...
                    return (
                      <div key={l.id} onClick={()=>{ setSelected(l); setActiveTab('lesson'); setTimeout(()=>document.getElementById('lesson-content')?.scrollIntoView({behavior:'smooth'}),100) }}
            ...
            <div id="lesson-content">
              {!selected ? (
                ...
                <div style={{background:'white',borderRadius:16,boxShadow:'var(--shadow-sm)',border:'1.5px solid #e2e8f0',overflow:'hidden'}}>
                  ...
                  {/* Tab header */}
                  <div style={{background:'linear-gradient(135deg,var(--brand-pale),white)',padding:'20px 24px 0',borderBottom:'1px solid #f1f5f9'}}>
                    ...
                    <h2 style={{fontFamily:'var(--font-display)',color:'var(--brand-deep)',fontSize:'clamp(1.2rem,2.5vw,1.8rem)',margin:'0 0 8px',lineHeight:1.3}}>{selected.title}</h2>
                    {selected.scripture && (
                      <div style={{display:'inline-flex',alignItems:'center',gap:8,background:'var(--gold)',color:'white',padding:'5px 14px',borderRadius:20,fontSize:'0.8rem',fontWeight:700,marginBottom:12}}>
                        📜 {selected.scripture}
                      </div>
                    )}
                    ...
                    <div style={{display:'flex',gap:4,marginTop:4}}>
                      {tabs.map(tab => (
                        (tab.id === 'analysis' && !selected.analysis && !selected.analysis_points) ? null :
                        (tab.id === 'divine' && !selected.divine_message_title && !selected.divine_message_speaker && !selected.evening_title && !selected.evening_speaker) ? null :
                        <button key={tab.id} onClick={()=>setActiveTab(tab.id)}
                          style={{padding:'8px 18px',borderRadius:'10px 10px 0 0',border:'none',cursor:'pointer',
                            fontFamily:'var(--font-body)',fontSize:'0.82rem',fontWeight:700,
                            background:activeTab===tab.id?'white':'transparent',
                            color:activeTab===tab.id?'var(--brand-deep)':'var(--text-light)',
                  ...
                  {selected.pdf_url && (
                    <div style={{background:'#f0fdf4',borderBottom:'1px solid #bbf7d0',padding:'10px 24px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
                      <span style={{color:'#166534',fontSize:'0.82rem',fontWeight:600}}>PDF version available</span>
                      <a href={selected.pdf_url} target="_blank" rel="noreferrer"
                        style={{background:'#16a34a',color:'white',padding:'6px 16px',borderRadius:20,textDecoration:'none',fontWeight:700,fontSize:'0.8rem'}}>
                        Download
                      </a>
                    </div>
                  )}
                  ...
                  <div style={{padding:'24px 28px'}}>
                    ...
                    {activeTab === 'lesson' && (
                      <div>
                        {selected.summary && (
                          <div style={{background:'var(--brand-pale)',borderLeft:'4px solid var(--brand-light)',borderRadius:'0 10px 10px 0',padding:'16px 20px',marginBottom:24,fontStyle:'italic',color:'var(--brand-deep)',lineHeight:1.8,fontSize:'0.95rem'}}>
                            {selected.summary}
                          </div>
                        )}
                        {selected.body ? (
                        ...
                        {selected.discussion_questions && (
                          <div style={{marginTop:28,background:'#fffbf0',borderRadius:12,padding:'20px 24px',border:'1.5px solid #fcd34d'}}>
                    ...
                    {activeTab === 'analysis' && (
                      <div>
                        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:20}}>
                          <div style={{width:40,height:40,borderRadius:12,background:'linear-gradient(135deg,var(--brand-deep),var(--brand-mid))',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'1.2rem'}}>🔍</div>
                          <div>
                            <div style={{fontFamily:'var(--font-display)',fontWeight:800,color:'var(--brand-deep)',fontSize:'1rem'}}>Detailed Analysis</div>
                            <div style={{fontSize:'0.75rem',color:'var(--text-light)'}}>In-depth study and commentary</div>
                          </div>
                        </div>
                        ...
                        {selected.analysis ? (
                          ...
                          <div style={{textAlign:'center',padding:'48px 20px',color:'var(--text-light)'}}>
                            ...
                            <div style={{fontWeight:600,marginBottom:6}}>No analysis yet</div>
                            <div style={{fontSize:'0.85rem'}}>Detailed analysis for this lesson will appear here when added.</div>
                          </div>
                        )}

                        {selected.analysis_points && (
                          <div style={{marginTop:28}}>
                            <h4 style={{fontFamily:'var(--font-display)',color:'var(--brand-deep)',fontSize:'1rem',marginBottom:16}}>Key Points</h4>
                            <div style={{display:'flex',flexDirection:'column',gap:12}}>
                              {selected.analysis_points.split('\n').filter(Boolean).map((pt,i)=>(
                                <div key={i} style={{display:'flex',gap:12,alignItems:'flex-start',background:'var(--brand-pale)',borderRadius:10,padding:'12px 16px'}}>
                                  <span style={{width:26,height:26,borderRadius:'50%',background:'var(--brand-light)',color:'white',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.75rem',fontWeight:900,flexShrink:0}}>{i+1}</span>
                                  <span style={{color:'var(--brand-deep)',fontSize:'0.92rem',lineHeight:1.6}}>{pt.replace(/^\d+\.\s*/,'')}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                    ...
                    {activeTab === 'divine' && (
                      <div>
                        {(selected.divine_message_title || selected.divine_message_speaker) && (
                          <div style={{background:'linear-gradient(135deg,var(--brand-pale),#f0f7ff)',borderRadius:14,padding:'22px 26px',border:'1.5px solid #bfdbfe',marginBottom:16}}>
                        ...
                        {(selected.evening_title || selected.evening_speaker) && (
                          <div style={{background:'linear-gradient(135deg,#1e1b4b,#2d2b5e)',borderRadius:14,padding:'22px 26px',border:'1.5px solid #4338ca'}}>
                            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
                              <span style={{fontSize:'1.5rem'}}>🌙</span>
                              <div>
                  ...
                  <div style={{padding:'14px 24px',borderTop:'1px solid #f1f5f9',display:'flex',justifyContent:'space-between',gap:12}}>
                    {(() => {
                      ...
                      return (
                        <div style={{display:'flex',justifyContent:'space-between',width:'100%',gap:12}}>
                          {prev ? <button onClick={()=>setSelected(prev)} style={{background:'none',border:'1.5px solid #e2e8f0',borderRadius:10,padding:'9px 18px',cursor:'pointer',color:'var(--text-mid)',fontFamily:'var(--font-body)',fontSize:'0.82rem'}}>← {prev.title.length>25?prev.title.slice(0,25)+'…':prev.title}</button> : <div/>}
                          {next ? <button onClick={()=>setSelected(next)} style={{background:'none',border:'1.5px solid #e2e8f0',borderRadius:10,padding:'9px 18px',cursor:'pointer',color:'var(--text-mid)',fontFamily:'var(--font-body)',fontSize:'0.82rem'}}>{next.title.length>25?next.title.slice(0,25)+'…':next.title} →</button> : <div/>}
                        </div>
                      )
                    })()}
                  </div>
