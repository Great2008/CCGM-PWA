...
export default function Hymnal() {
  ...
  const fetchFresh = useCallback(async (cached) => {
    try {
      const { data } = await supabase.from('hymns')
  ...
  const selectedIdx = filtered.findIndex(h => h.id === selected?.id)
  const prevHymn    = filtered[selectedIdx + 1]
  const nextHymn    = filtered[selectedIdx - 1]
  ...
  if (offline && hymns.length === 0) return (
  ...
  return (
    ...
    <div style={{ overflowX: 'hidden', width: '100%' }}>
      ...
      {/* Hero */}
      <div className="hm-hero" style={{ background: 'linear-gradient(135deg,var(--brand-deep),var(--brand-mid))', padding: 'clamp(90px,14vw,130px) 5% 56px', textAlign: 'center' }}>
        <span className="section-label" style={{ color: 'var(--gold)' }}>Worship & Praise</span>
        <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(2rem,5vw,3rem)', color: 'white', margin: '8px 0 16px' }}>🎵 Hymnal</h1>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column' }} onClick={() => setShowList(false)}>
          <div style={{ marginTop: 'auto', background: 'var(--white, white)', borderRadius: '20px 20px 0 0', maxHeight: '82vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '8px 18px 14px', borderBottom: '1px solid #f0fdf4' }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search hymns or authors..."
              ...
              {filtered.map(h => {
                ...
                return (
                  <div key={h.id} id={'hm-item-' + h.id} className="hm-item" onClick={() => selectHymn(h)}
                    ...
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: isSel ? 'var(--brand-base)' : 'var(--brand-pale)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '0.75rem', color: isSel ? 'white' : 'var(--brand-mid)', flexShrink: 0 }}>
                      {h.sort_order ?? '—'}
                    </div>
            ...
            {!selected ? (
              ...
              <div className="hm-card" style={{ background: 'var(--white, white)', borderRadius: 16, boxShadow: 'var(--shadow-sm)', border: '1.5px solid #d1fae5', overflow: 'hidden' }}>
                ...
                <div style={{ background: 'linear-gradient(135deg,var(--brand-deep),var(--brand-mid))', padding: 'clamp(20px,4vw,32px) clamp(18px,4vw,32px) 0' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 10 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {selected.category && (
                        <span style={{ display: 'inline-block', marginTop: 8, fontSize: '0.7rem', fontWeight: 700, padding: '3px 12px', borderRadius: 20, background: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.85)' }}>{selected.category}</span>
                      )}
                    </div>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'flex-start', flexShrink: 0 }}>
                      {user && (
                        <button onClick={e => toggleFavourite(e, selected.id)} disabled={favLoading}
                          style={{ background: favourites.has(selected.id) ? 'rgba(245,158,11,0.25)' : 'rgba(255,255,255,0.12)', border: `1px solid ${favourites.has(selected.id) ? 'var(--gold)' : 'rgba(255,255,255,0.25)'}`, color: 'white', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: '0.88rem', fontFamily: 'var(--font-body)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, transition: 'all 0.2s' }}>
                    ...
                    <>
                      {verses.map((verse, i) => (
                        <div key={verse.number ?? i} style={{ marginBottom: 28 }}>
                          <div style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--brand-light)', marginBottom: 8 }}>
                            Verse {verse.number ?? i + 1}
                          </div>
                          <p style={{ fontSize: fontSize + 'px', lineHeight: 2.1, color: 'var(--text-dark)', whiteSpace: 'pre-line', fontStyle: 'italic', margin: 0 }}>
                            {verse.text}
                          </p>
                        </div>
                      ))}
                      {selected.chorus && (
                        <div style={{ background: 'var(--brand-pale)', borderRadius: 12, padding: '18px 22px', borderLeft: '4px solid var(--gold)', marginBottom: 24 }}>
                          ...
                          <p style={{ fontSize: fontSize + 'px', lineHeight: 2.1, color: 'var(--brand-deep)', whiteSpace: 'pre-line', fontWeight: 700, fontStyle: 'italic', margin: 0 }}>
                            {selected.chorus}
                          </p>
                        </div>
                      )}
                ...
                <div style={{ padding: '14px clamp(16px,4vw,28px)', borderTop: '1px solid #f0fdf4', display: 'flex', justifyContent: 'space-between', gap: 12 }}>
                  {prevHymn ? (
                    <button onClick={() => selectHymn(prevHymn)} style={{ background: 'none', border: '1.5px solid #d1fae5', borderRadius: 10, padding: '10px 16px', cursor: 'pointer', color: 'var(--text-mid)', fontFamily: 'var(--font-body)', fontSize: (fontSize - 3) + 'px', flex: 1, textAlign: 'left', lineHeight: 1.4 }}>
                      {'← #' + prevHymn.sort_order + ' ' + (prevHymn.title.length > 25 ? prevHymn.title.slice(0, 25) + '…' : prevHymn.title)}
                    </button>
                  ) : <div />}
                  {nextHymn ? (
                    <button onClick={() => selectHymn(nextHymn)} style={{ background: 'none', border: '1.5px solid #d1fae5', borderRadius: 10, padding: '10px 16px', cursor: 'pointer', color: 'var(--text-mid)', fontFamily: 'var(--font-body)', fontSize: (fontSize - 3) + 'px', flex: 1, textAlign: 'right', lineHeight: 1.4 }}>
                      {'#' + nextHymn.sort_order + ' ' + (nextHymn.title.length > 25 ? nextHymn.title.slice(0, 25) + '…' : nextHymn.title) + ' →'}
                    </button>
                  ) : <div />}
                </div>
