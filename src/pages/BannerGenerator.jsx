...
const FORMATS = [
  ...
  { id: 'instagram', label: 'Instagram',       w: 1080, h: 1350 },
...
function drawBanner(canvas, { fmt, style, fields, logoImg }) {
  ...
  grad.addColorStop(1, isDark ? '#1a3a22' : '#bbf7d0')
  ctx.fillStyle = grad
  ...
  const details = [
    fields.date  ? { icon: '📅', val: fields.date  } : null,
    fields.time  ? { icon: '🕘', val: fields.time  } : null,
    fields.venue ? { icon: '📍', val: fields.venue } : null,
  ].filter(Boolean)
  ...
  for (const d of details) {
    ...
    ctx.textAlign = align
    ctx.fillText(d.icon + '  ' + d.val, isLandscape ? pad : midX, y)
    y += detSize * 1.7
  }
        ...
        <div style={{ maxWidth: 640, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 16, boxShadow: 'var(--shadow-md)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <span style={{ fontSize: '0.74rem', color: 'var(--text-muted)' }}>{fmt.w}x{fmt.h}px</span>
            </div>
            <div style={{ width: '100%', background: '#e2e2e2', borderRadius: 10, padding: 8, lineHeight: 0 }}>
              <canvas
                ref={canvasRef}
                style={{ width: '100%', height: 'auto', display: 'block', borderRadius: 6, boxShadow: '0 4px 20px rgba(0,0,0,0.18)' }}
              />
            </div>
          </div>
          ...
          <button onClick={download} style={{
            ...
            cursor: 'pointer', letterSpacing: '0.5px', boxShadow: 'var(--shadow-md)',
          }}>
            ⬇ Download PNG
          </button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {FORMATS.map(f => (
                <button key={f.id} onClick={() => setFmtId(f.id)} style={{
                  ...
                  <span>{f.label}</span>
                  <span style={{ color: 'var(--text-muted)', fontSize: '0.74rem' }}>{f.w}x{f.h}</span>
                </button>
              ))}
            </div>
          </Card>
